...
The system uses a **\*\*monolithic, single-process design\*\*** for the Metrics Server (\`aggregator\_server.py\`) to maximize efficiency and minimize network overhead between components.
...
\* The final update is a single, instantaneous **\*\*reference swap\*\*** (\`self.current\_data \= new\_snapshot\`), protected only briefly by an \`asyncio.Lock\`.  
...
1. **Start the Telemetry Generator (Producer):**  
   ...
   * **Edge Case Testing:** To simulate ingestion failures, you can run the Generator in a specific mode (e.g., python generator.py ERROR\_500) or call the /counters endpoint with a mode query parameter (e.g., http://127.0.0.1:9001/counters?mode=CORRUPT).  
...
| /telemetry/GetMetric | GET | Fetch a specific metric value for a given switch ID. | http://127.0.0.1:8080/telemetry/GetMetric?switch\_id=SW-05\&metric\_name=Latency\_Avg\_uSec |
